...
def poll_editor_main(request, poll = None):
    if poll==None:
        session_prefix = "new"
    else:
        session_prefix = "edit"
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session[session_prefix+'_poll_title']=form.cleaned_data['poll_title']
            request.session[session_prefix+'_poll_text']=form.cleaned_data['poll_text']
            request.session[session_prefix+'_poll_alternative_count']=alternative_count
            alternatives=[]
            for i in range(1,alternative_count+1):
                alternatives.append(form.cleaned_data['alternative'+str(i)])
            request.session[session_prefix+'_poll_alternatives']=alternatives
            request.session[session_prefix+'_poll_type']=form.cleaned_data['poll_type']
            request.session[session_prefix+'_poll_page_index']=2
...
def poll_editor_summary_and_additional_options(request, poll = None):
    ...
    if request.method == 'POST' and 'go_back' in request.POST:
        form=CreatePollFormMain(
            poll_title=request.session[session_prefix+'_poll_title'],
            poll_text=request.session[session_prefix+'_poll_text'],
            poll_type=request.session[session_prefix+'_poll_type'],
            alternatives=request.session[session_prefix+'_poll_alternatives']
            )
        request.session[session_prefix+'_poll_page_index']=1
