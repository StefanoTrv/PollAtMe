...
class RevoteSessionCleaner:
    ...
    def __call__(self, request: HttpRequest):
        if 'preference_id' in request.session and 'vote/singlepreference' not in request.get_full_path() and 'vote/majorityjudgment' not in request.get_full_path() and 'help/majorityjudgment' not in request.get_full_path() and 'help/schulze_method/' not in request.get_full_path():
