...
class TokenizedPoll(Poll):
    ...
    def user_has_already_voted(self, **kwargs) -> bool:
        return Token.objects.filter(token=kwargs['token'].lower(),poll=self).first().used # type: ignore
    ...
    def set_authentication_method_as_used(self, **kwargs) -> None:
        if self.failed_authentication(**kwargs):
            ...
            token_object = Token.objects.filter(token=kwargs['token'].lower(),poll=self).first()
