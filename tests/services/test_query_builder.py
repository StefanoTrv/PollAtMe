...
class TestQueryBuilder(TestCase):
    def setUp(self) -> None:
        ...
        polls = [
            {'title': 'A', 'text': 'A', 'default_type': 1, 'start': timezone.now(
            ) - timedelta(weeks=2), 'end': timezone.now() - timedelta(weeks=1), 'author': u},  # concluso
            {'title': 'B', 'text': 'B', 'default_type': 3, 'start': timezone.now(
            ) - timedelta(weeks=2), 'end': timezone.now() - timedelta(weeks=1), 'author': u},  # concluso
            {'title': 'C', 'text': 'C', 'default_type': 2, 'start': timezone.now(
            ) - timedelta(weeks=1), 'end': timezone.now() + timedelta(weeks=2), 'author': u},  # attivo
            {'title': 'D', 'text': 'D', 'default_type': 1, 'start': timezone.now(
            ) - timedelta(weeks=1), 'end': timezone.now() + timedelta(weeks=2), 'author': u},  # attivo
            {'title': 'E', 'text': 'E', 'default_type': 3, 'start': timezone.now(
            ) + timedelta(weeks=1), 'end': timezone.now() + timedelta(weeks=2), 'author': u},  # non ancora attivo
            {'title': 'F', 'text': 'F', 'default_type': 2, 'start': timezone.now(
            ), 'end': timezone.now(), 'author': u},  # senza opzioni
        ]
    ...
    def test_empty_query(self):
        ...
        all_polls = [p for p in Poll.objects.all(
        ) if p.alternative_set.count() > 0]
        assert_that(all_polls).is_equal_to(query)

    def test_title(self):
        query = SearchPollQueryBuilder().title_filter("A").search()
        expected_polls = [p for p in Poll.objects.filter(
            title__icontains="A") if p.alternative_set.count() > 0]
    ...
    def test_type(self):
        query = SearchPollQueryBuilder().type_filter(
            Poll.PollType.MAJORITY_JUDGMENT).search()
        expected_polls = [p for p in Poll.objects.filter(
    ...
    def test_range_start(self):
        ...
        expected_polls = [p for p in Poll.objects.filter(
            start__gte=start) if p.alternative_set.count() > 0]
        assert_that(expected_polls).is_equal_to(query)

        end = timezone.now()
        query = SearchPollQueryBuilder().start_range_filter(end=end).search()
        expected_polls = [p for p in Poll.objects.filter(
            start__lte=end) if p.alternative_set.count() > 0]
        assert_that(expected_polls).is_equal_to(query)
    ...
    def test_range_end(self):
        ...
        expected_polls = [p for p in Poll.objects.filter(
            end__gte=start) if p.alternative_set.count() > 0]
        assert_that(expected_polls).is_equal_to(query)

        end = timezone.now()
        query = SearchPollQueryBuilder().end_range_filter(end=end).search()
        expected_polls = [p for p in Poll.objects.filter(
            end__lte=end) if p.alternative_set.count() > 0]
        assert_that(expected_polls).is_equal_to(query)

    def test_complex(self):
        query = SearchPollQueryBuilder().title_filter(
            "B").status_filter("NOT_STARTED").search()
        expected_polls = [p for p in Poll.objects.filter(id__in=[
            poll.id
